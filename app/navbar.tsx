    ...
    <nav className="">
      <div className="mx-auto flex items-center h-full">
        ...
        <div
          ...
          className="font-bold whitespace-nowrap px-6 py-3 border-r border-nav_border dark:border-dark_nav_border "
        >
          <Link href="/" className={linkStyle}>GRANT DONG</Link>
        </div>
        
        {/* Zebra Stripes */}
        {/* unsure if i should keep this */}
        <div
          className="flex-grow h-8 mx-2 bg-[image:var(--zebra)] dark:bg-[image:var(--zebra-dark)]" // hidden sm:flex
          style={{ 
            ['--zebra' as string]: zebraStyleLight,
            ['--zebra-dark' as string]: zebraStyleDark,
          }}
        ></div>
        ...
        <div className="flex border-l border-nav_border dark:border-dark_nav_border items-center px-6 py-3 sm:hidden ml-auto text-xl font-bold transition-transform duration-300">
          ...
          >
              <div className={`relative w-6 h-6 flex flex-col justify-center items-center`}>
                <span className={`absolute left-0 h-[2px] bg-nav_text dark:bg-dark_nav_text transition-all duration-300 ${menuOpen ? 'top-3 left-2 w-2 opacity-0' : 'top-1 left-0 w-6 opacity-100'}`}></span>
                ...
                <span className={`absolute left-0 w-6 h-[2px] bg-nav_text dark:bg-dark_nav_text transition-all duration-200 ${menuOpen ? 'top-3 -rotate-45' : 'top-3'}`}></span>
                <span className={`absolute left-0 h-[2px] bg-nav_text dark:bg-dark_nav_text transition-all duration-300 ${menuOpen ? 'top-3 left-2 w-2 opacity-0' : 'top-5 left-0 w-6 opacity-100'}`}></span>
              </div>
          </button>
        </div>
